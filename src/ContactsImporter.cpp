...
void ContactsImporter::importVcard(const QStringList &filenames)
{
	int total = 0, invalid = 0, ignored = 0, overwritten = 0, merged = 0, added = 0;
	...
	try {
		...
		for (int fileIndex = 0; fileIndex < filenames.size(); ++fileIndex) {
			const QString &filename = filenames[fileIndex];

			QFile file(filename);
			if (!file.open(QIODevice::ReadOnly)) {
				std::stringstream ss;
				ss << "Error opening file " << filename.toStdString();
			...
			for (int i = 0;; ++i, ++total) {
				...
				systemProgressDialog->setProgress(fileIndex * 100 / filenames.size());
				systemProgressDialog->setStatusDetails(QString(tr("%1 contact(s) processed")).arg(total));
				systemProgressDialog->show();
			}
		}
	} catch (std::runtime_error &ex) {
		Utilities::showSystemToast(this, ex.what());
	} catch (break_exception &ex) {
		// Process was interrupted by the user, no action required.
	}
...
void ContactsImporter::importCsv(const QString &filename)
{
